...
export default class ReactMic extends Component {
    ...
    if (prevProps.record !== record) {
      if (record) {
        if (microphoneRecorder) {
          microphoneRecorder.start().then(onStart).catch((e) => {
            console.error(e)
          });
          if (chunkInterval) {
          ...
          chunkInterval = setInterval(() => {
            microphoneRecorder.getMp3().then(([_, blob]) => {
        ...
        microphoneRecorder.stop().getMp3().then(([_, blob]) => {
          const soundDetected = AudioContext.getSoundDetected()
          AudioContext.setSoundDetected(false)
  ...
  componentDidMount() {
    ...
    if (audioElem) {
      ...
      this.setState({
        microphoneRecorder: new MicRecorder(
          // onStart,
          // onStop,
          // onSave,
          // onData,
