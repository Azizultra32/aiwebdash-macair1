...
After setup finishes, HTTP and Git network access remain available for tools like `curl`, `gh`, and `git`. The `.codex/setup.sh` script installs dependencies and fetches open pull requests. It first checks for an `origin` remote and falls back to the `REPO_URL` environment variable if one isn't configured.
